...
:::tip 进阶功能
如果你使用了 [内容代码分离](/guide/content-separation/) 功能，部署时需要额外配置环境变量。
:::

使用内容分离时，需要在部署平台配置以下环境变量：
- `ENABLE_CONTENT_SYNC=true`
- `CONTENT_REPO_URL=你的内容仓库地址`
- `USE_SUBMODULE=true`

**详细配置请查看**:
- [内容代码分离 - 部署配置](/guide/content-separation/#部署配置) - Vercel等平台详细配置方法

:::warning 注意
内容仓库更新默认不会触发部署，需要配置 [自动构建触发](/guide/auto-build-trigger/) 才能实现自动部署。
:::
